<!-- component -->
<!-- This is an footer component -->
<div class="mt-10 bg-primary">
    <div class="max-w-2xl py-10 mx-auto text-white">
        <div class="text-center">
            <h1 class="mb-3 text-2xl"> Sistem Manajemen Event - Universitas Teknokrat Indonesia</h1>
        </div>
        <div class="flex flex-col items-center text-sm mt-28 md:flex-row md:justify-between">
            <p class="order-2 mt-8 md:order-1 md:mt-0"> &copy; SIMANEV UTI, 2023. </p>
            <div class="order-1 md:order-2">
                <span class="px-2">About us</span>
                <span class="px-2 border-l">Event</span>
            </div>
        </div>
    </div>
</div>
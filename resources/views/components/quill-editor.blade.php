<div 
    ...
    x-cloak>
    @if($label ?? null)
        <label for="{{ $name }}" class="block mb-1 font-semibold text-gray-700 form-label">
            {{ $label }}
            @if($optional ?? null)
    ...
    <div class="relative {{ $errors->has($name) ? 'ql-editor-haserror' : '' }}">
        ...
        <div class="absolute left-0 right-0 z-20 w-full pl-px pr-px bg-transparent" style="top: 38px;">
          <div id="quillProgressBar" class="h-1 text-xs leading-none bg-green-600" style="width: 0%"></div>
        </div>

        <textarea class="hidden" name="{{ $name }}" :value="content"></textarea>
        <div x-ref="quillEditor" x-model="content" class="h-auto min-h-full bg-white">
            {!! old($name, $value ?? '') !!}
        </div>

        @error($name)
            <svg class="absolute z-10 w-5 h-5 text-red-600 fill-current" style="top: 12px; right: 12px"
                xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                <path
                    d="M11.953,2C6.465,2,2,6.486,2,12s4.486,10,10,10s10-4.486,10-10S17.493,2,11.953,2z M13,17h-2v-2h2V17z M13,13h-2V7h2V13z" />
            </svg>
            <div class="block mt-2 text-sm leading-tight text-red-600">{{ $message }}</div>
        @enderror
    </div>
</div>
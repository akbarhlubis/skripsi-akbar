                    ...
                    <form action="{{ route('event.update', $event) }}" class="" method="POST"
                            ...
                            <div class="flex flex-col w-full">
                                ...
                                <select name="category_id" id="category_id" class="rounded-md">
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}">{{ old('category', $category->name) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col gap-2 py-2" x-data="{ show: false }">
                            ...
                            <input x-show="show" type="url" id="link" name="link"
                                ...
                                name="link" class="block w-full mb-4 rounded-md" value="{{ old('link', $event->link) }}">
                            <input x-show="show" type="url" id="embed" name="embed"
                            placeholder="Link SpreadSheet (Google Sheet, dll))"
                            name="embed" class="block w-full mb-4 rounded-md" value="{{ old('link', $event->embed) }}">
                        </div>
                        ...
                        <input type="text" id="description" name="description" class="block w-full mb-4 rounded-md"
                            value="{{ old('description', $event->description) }}">

                        <label for="quota" class="block">Kuota Peserta</label>
                        <input type="text" id="quota" name="quota" class="block w-full mb-4 rounded-md"
